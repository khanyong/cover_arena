...
// 1. versionHistory 업데이트
if (!content.includes('"v6.3"')) {
  content = content.replace(/versionHistory:\s*\[([^\]]+)\]/, (match, versions) => {
    return `versionHistory: [${versions.trim()}, "v6.1", "v6.2", "v6.3"]`;
  });
}

const now = new Date().toISOString().slice(0, 16).replace('T', ' ');

// 2. 프롤로그 삽입 (백슬래시 완벽 보호)
const prologueStr = `
    {
      number: 0,
      title: "프롤로그",
...
const p1Start = content.indexOf('텅 빈 제네바의 연구실.');
if (p1Start !== -1) {
  content = content.slice(0, p1Start) + '3년 후, ' + content.slice(p1Start);
}

// 4. 구 1막의 불필요한 회상 씬 제거 및 구 2막 챕터 편입
// act1-ch1-p1이 끝나는 닫는 중괄호와 콤마를 찾는다.
...
// 구 2막 1장의 제목 "제 1 장: 스코틀랜드의 은둔자와 3년 만의 재회" 바로 앞까지를 날린다.
const act2Ch1Regex = /title: "제 1 장: 스코틀랜드의 은둔자와 3년 만의 재회",/;
const act2Ch1Match = content.match(act2Ch1Regex);

// 그 챕터의 number: 1 선언이 그 위에 있을 것이다.
// 대략 `        {\n          number: 1,\n          title: "제 1 장: 스코틀랜드...`
const act2Ch1BlockStartStr = '        {\n          number: 1,\n          title: "제 1 장: 스코틀랜드의 은둔자와 3년 만의 재회",';
const deleteEndIdx = content.indexOf(act2Ch1BlockStartStr);
...
if (deleteEndIdx !== -1) {
  ...
  const afterDelete = content.slice(deleteEndIdx + act2Ch1BlockStartStr.length);
  content = beforeDelete + replacement + afterDelete;
}

// 5. 편입된 구 2막 챕터들의 번호 밀기 (2~7장까지)
content = content.replace(/number: 2,(\n\s*title: "제 2 장: 암흑)/g, 'number: 3,$1');
content = content.replace(/title: "제 2 장: 암흑/g, 'title: "제 3 장: 암흑');

content = content.replace(/number: 3,(\n\s*title: "제 3 장: 공간)/g, 'number: 4,$1');
content = content.replace(/title: "제 3 장: 공간/g, 'title: "제 4 장: 공간');

content = content.replace(/number: 4,(\n\s*title: "제 4 장: 초공간)/g, 'number: 5,$1');
content = content.replace(/title: "제 4 장: 초공간/g, 'title: "제 5 장: 초공간');

content = content.replace(/number: 5,(\n\s*title: "제 5 장: 3광년)/g, 'number: 6,$1');
content = content.replace(/title: "제 5 장: 3광년/g, 'title: "제 6 장: 3광년');

content = content.replace(/number: 6,(\n\s*title: "제 6 장: 8개월)/g, 'number: 7,$1');
content = content.replace(/title: "제 6 장: 8개월/g, 'title: "제 7 장: 8개월');

content = content.replace(/number: 7,(\n\s*title: "제 7 장: 다자간)/g, 'number: 8,$1');
content = content.replace(/title: "제 7 장: 다자간/g, 'title: "제 8 장: 다자간');
...
// 에필로그 번호를 5로 올림
content = content.replace(/number: 4,\n\s*title: "에필로그: 텐서의 창세기/, 'number: 5,\n      title: "에필로그: 텐서의 창세기');

fs.writeFileSync(targetPath, content);
console.log("Successfully rebuilt perfect novel structure!");
