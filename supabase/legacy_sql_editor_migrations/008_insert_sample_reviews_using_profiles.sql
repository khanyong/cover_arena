-- profiles 테이블의 사용자를 활용한 샘플 리뷰 데이터 생성
...
BEGIN
  ...
  IF existing_users IS NULL OR array_length(existing_users, 1) = 0 THEN
    ...
    IF existing_users IS NULL OR array_length(existing_users, 1) = 0 THEN
      RAISE NOTICE 'Failed to create sample users. Please check profiles table structure.';
      RETURN;
    END IF;
  END IF;
  ...
  LOOP
    ...
    FOR i IN 1..review_count LOOP
      ...
      -- 일부 리뷰에 반응 추가 (30% 확률)
      IF new_review_id IS NOT NULL AND random() < 0.3 THEN
        -- 다른 사용자가 도움됨 반응
