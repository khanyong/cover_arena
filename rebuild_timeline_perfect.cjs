...
// versionHistory 갱신 (v6.1, v6.2 동시 추가)
...
if (actsIndex !== -1) {
  ...
  // 단, 여기서 템플릿 리터럴 내부의 문자열 '\n'을 실제 줄바꿈 기호로 바꾼다.
  const finalPrologueStr = prologueStr.replace(/\\n/g, '\n');
  content = content.slice(0, insertPos) + '\n' + finalPrologueStr + content.slice(insertPos);
}
...
console.log('Successfully perfectly rebuilt v6.2 timeline.');
