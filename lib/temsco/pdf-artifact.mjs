...
export function createPdfArtifact(config) {
  ...
  function readManifest(root = process.cwd()) {
    ...
    if (Object.hasOwn(manifest, 'buildCompatibility')) {
      const compatibility = manifest.buildCompatibility
      if (!compatibility || typeof compatibility !== 'object' || Array.isArray(compatibility) ||
          Object.keys(compatibility).sort().join(',') !== 'baseSourceHash,reason,sourceHash' ||
          !/^[a-f0-9]{64}$/.test(compatibility.sourceHash ?? '') ||
          compatibility.baseSourceHash !== manifest.sourceHash ||
          typeof compatibility.reason !== 'string' || !compatibility.reason.trim() || compatibility.reason.length > 1000) {
        throw artifactError('PDF 빌드 호환성 기록이 올바르지 않음')
      }
    }
    return manifest
  }
  ...
  function verifyArtifact(root = process.cwd()) {
    ...
    // A reviewed build-only change may pin one additional exact source graph.
    // The original PDF identity and all byte/page checks remain unchanged.
    if (manifest.sourceHash !== sourceHash && manifest.buildCompatibility?.sourceHash !== sourceHash) {
      throw artifactError('현재 코드와 PDF의 소스 버전 불일치')
    }
    let bytes
    try {
