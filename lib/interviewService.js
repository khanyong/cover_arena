...
/**
 * Get all questions for multiple question IDs at once (batch query)
 * @param {string[]} questionIds - Array of question IDs
 * @returns {Promise<{data, error}>}
 */
export async function getAllQuestionsByIds(questionIds) {
  const { data, error } = await supabase
    .from('univ_user_interview_questions')
    .select('*')
...
/**
 * Get all answers for multiple question IDs at once (batch query)
 * Returns latest 3 versions per question
 * @param {string[]} questionIds - Array of question IDs
 * @returns {Promise<{data, error}>}
 */
export async function getAllAnswersByIds(questionIds) {
  const { data, error } = await supabase
    .from('univ_user_interview_answers')
    .select('*')
