-- site_score 필드의 크기를 늘려서 오버플로우 문제 해결

-- 1. coversong_rank_history 테이블의 site_score 컬럼 타입 변경
ALTER TABLE coversong_rank_history 
ALTER COLUMN site_score TYPE NUMERIC(20, 2);

-- 2. 확인 (선택사항 - 테이블 구조 확인)
-- SELECT column_name, data_type FROM information_schema.columns 
-- WHERE table_name = 'coversong_rank_history';

-- 3. 이제 초기 데이터 생성 다시 시도
-- 오늘의 순위 기록
DO $$
DECLARE
...
BEGIN
  ...
  IF v_competition_id IS NOT NULL THEN
    ...
    SELECT 
      ...
      COALESCE(v.site_score, 0)::NUMERIC(20, 2),  -- NULL 처리 및 타입 캐스팅
      COALESCE(v.arena_likes, 0),
      COALESCE(v.likes, 0),
      CURRENT_DATE
...
BEGIN
  ...
  IF v_competition_id IS NOT NULL THEN
    ...
    SELECT 
      ...
      GREATEST(0, COALESCE(v.site_score, 0) - (RANDOM() * 10))::NUMERIC(20, 2),
      GREATEST(0, COALESCE(v.arena_likes, 0) - FLOOR(RANDOM() * 5)::INTEGER),
      COALESCE(v.likes, 0),
