...
(async () => {
  ...
  try {
    ...
    if (route === '/temsco/deck-v3') {
      ...
      result.copyReview = await page.locator('section.temsco-slide').evaluateAll(slides => slides.map((slide, index) => {
        const text = slide.textContent.replace(/\s+/g, ' ').trim();
        return { page: index + 1, text, proseEndings: text.match(/[^.!?·]{0,55}(?:합니다|입니다|됩니다|했습니다|않습니다|있습니다|없습니다|아닙니다|해야 합니다|였습니다|봅니다|남습니다)[.!?]?/g) || [] };
      }));
      result.dividers = await page.locator('section[data-chapter-divider]').evaluateAll(slides => slides.map(slide => ({ numeral: slide.dataset.chapterDivider, id: slide.parentElement.id, title: slide.querySelector('h2').textContent })));
      result.contentsNavigation = [];
      for (const numeral of ['I','II','III','IV','V']) {
        await page.getByLabel('슬라이드 선택', { exact: true }).selectOption('1');
        const link = page.locator('#slide-2 a').filter({ hasText: new RegExp('^' + numeral + '\\.') });
        const href = await link.getAttribute('href');
        await link.click();
        const divider = page.locator('section[data-chapter-divider="' + numeral + '"]');
        result.contentsNavigation.push({ numeral, href, visible: await divider.isVisible(), selected: await page.getByLabel('슬라이드 선택', { exact: true }).inputValue() });
      }
