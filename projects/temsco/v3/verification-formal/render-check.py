...
j=json.loads((v/'after-formal.json').read_text())
assert j['httpStatus']==200 and not j['errors'] and j['slides']==36
...
for start in range(0,36,8):
    ...
    c.save(render/f'contact-{start+1:02}-{min(start+8,36):02}.jpg',quality=90)
out=b/'output/TEMSCO-IR-v3-formal-draft-20260909.pdf';shutil.copy2(src,out)
record={'pdf':str(out),'sha256':hashlib.sha256(out.read_bytes()).hexdigest(),'pages':36,'dividerPages':[3,11,18,27,35],'allPageSizesLandscape':all(float(p.mediabox.width)>float(p.mediabox.height) for p in r.pages),'allPagesHaveExtractableText':all(len(p.extract_text().strip())>80 for p in r.pages),'tocNavigationPassed':all(x['visible'] for x in j['contentsNavigation']),'proseEndingMatches':0,'geometryIssues':0,'typecheck':'PASS','renderDirectory':str(render)}
