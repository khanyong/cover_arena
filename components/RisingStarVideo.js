...
export default function RisingStarVideo({ videos, onVideoClick }) {
  // 순위 상승폭이 가장 큰 비디오들 찾기 (동률 포함)
  const getRisingStarVideos = () => {
    ...
    // 최대 상승폭과 동일한 상승폭을 가진 모든 비디오 반환
    const maxRankIncrease = sortedVideos[0].previous_rank - sortedVideos[0].rank;
    return sortedVideos.filter(video => 
      (video.previous_rank - video.rank) === maxRankIncrease
    );
  };

  const risingStarVideos = getRisingStarVideos();

  if (!risingStarVideos || risingStarVideos.length === 0) {
  ...
  const maxRankIncrease = risingStarVideos[0].previous_rank - risingStarVideos[0].rank;

  return (
    <div className="mb-8">
      <h2 className="text-2xl font-bold text-white mb-4 flex items-center">
        🔥 순위 급상승 1위
        {risingStarVideos.length > 1 && (
          <span className="ml-2 text-sm text-orange-300">({risingStarVideos.length}개 동률)</span>
        )}
        <span className="ml-2 text-sm text-gray-300">(이전 업데이트 대비)</span>
      </h2>
      
      <div className="space-y-6">
        {risingStarVideos.map((risingStarVideo, index) => {
          const rankIncrease = risingStarVideo.previous_rank - risingStarVideo.rank;
          
          return (
            <div key={risingStarVideo.id} className="bg-gradient-to-br from-red-900/30 to-orange-900/30 backdrop-blur-sm rounded-lg p-6 shadow-lg border border-red-700/30">
              {risingStarVideos.length > 1 && (
                <div className="flex items-center justify-between mb-4">
                  <span className="text-orange-300 font-bold text-lg">공동 1위</span>
                  <span className="text-gray-300 text-sm">+{rankIncrease}단계 상승</span>
                </div>
              )}
